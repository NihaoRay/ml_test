...
# 帮助文档 https://zhuanlan.zhihu.com/p/260703124
...
TARGET_REPLACE_ITER = 50  # 目标网络更新频率(固定不懂的Q网络)
...
for i in range(180000):  # 设置400个episode
    print(f"<<<<<<<<<第{i}周期")
    state, _ = env.reset(seed=100)  # 重置环境
    episode_reward_sum = 0  # 初始化每个周期的reward值

    while True:
        env.render()  # 开启画面
        action = dqn.choose_action(state)  # 与环境互动选择action
        ...
        ########
        dqn.store_transition(state, action, r, next_state)  # 储存样本
        ...
        if done:
            print(f"episode:{i},reward_sum:{episode_reward_sum}, memory_counter:{dqn.memory_counter}")
            break
